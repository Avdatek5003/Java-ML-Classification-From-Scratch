...
public class KararAgaci implements IClassifier {
    ...
    public KararAgaci(int maksimumDerinlik) {
        if (maksimumDerinlik <= 0) {
            throw new IllegalArgumentException("Maksimum derinlik 0'dan büyük olmalıdır.");
        }
    ...
    public void train(List<UserRecord> egitimVerisi) {
        if (egitimVerisi == null || egitimVerisi.isEmpty()) {
            throw new IllegalArgumentException("Eğitim verisi boş olamaz.");
        }
    ...
    public String predict(UserRecord musteri) {
        if (kok == null) {
            throw new IllegalStateException("Model henüz eğitilmedi!");
        }
    ...
    private Dugum agacInsaEt(List<UserRecord> veriListesi, int mevcutDerinlik) {
        ...
        if (enIyiBolunme.kategorikMi) {
            return new Dugum(enIyiBolunme.ozellikIndeksi, enIyiBolunme.kategoriDegeri, solCocuk, sagCocuk);
        }
    ...
    private String ozyinelemeliTahminEt(UserRecord musteri, Dugum dugum) {
        ...
        if (dugum.kategorikMi) {
            boolean ayniKategori = musteri.getBrand().equalsIgnoreCase(dugum.kategoriDegeri);
            if (ayniKategori) {
        ...
        double deger = sayisalOzellikDegeriniAl(musteri, dugum.ozellikIndeksi);
    ...
    private EnIyiBolunme enIyiBolunmeyiBul(List<UserRecord> veriListesi) {
         ...
         */
        for (int ozellikIndeksi = 0; ozellikIndeksi < 2; ozellikIndeksi++) {
            List<Double> esikDegerleri = sayisalEsikleriOlustur(veriListesi, ozellikIndeksi);
            ...
            for (double esikDeger : esikDegerleri) {
                ...
                for (UserRecord u : veriListesi) {
                    if (sayisalOzellikDegeriniAl(u, ozellikIndeksi) <= esikDeger) {
                        solGrup.add(u);
                    } else {
                        sagGrup.add(u);
                    }
                ...
                if (solGrup.isEmpty() || sagGrup.isEmpty()) {
                    continue;
                }

                double agirlikliGini = agirlikliGiniHesapla(solGrup, sagGrup, veriListesi.size());

                if (agirlikliGini < enIyiGini) {
                    enIyiGini = agirlikliGini;
                    enIyiBolunmeSecimi = EnIyiBolunme.sayisalBolunme(ozellikIndeksi, esikDeger, solGrup, sagGrup);
                }
            }
        }
        ...
        for (String marka : markalar) {
            ...
            for (UserRecord u : veriListesi) {
                if (u.getBrand().equalsIgnoreCase(marka)) {
                    solGrup.add(u);
                } else {
                    sagGrup.add(u);
                }
            ...
            if (agirlikliGini < enIyiGini) {
                enIyiGini = agirlikliGini;
                enIyiBolunmeSecimi = EnIyiBolunme.kategorikBolunme(2, marka, solGrup, sagGrup);
            }
        }
     ...
     */
    private List<Double> sayisalEsikleriOlustur(List<UserRecord> veriListesi, int ozellikIndeksi) {
        TreeSet<Double> benzersizDegerler = new TreeSet<>();
        for (UserRecord u : veriListesi) {
            benzersizDegerler.add(sayisalOzellikDegeriniAl(u, ozellikIndeksi));
        }

        List<Double> degerler = new ArrayList<>(benzersizDegerler);
        List<Double> esikler = new ArrayList<>();

        for (int i = 0; i < degerler.size() - 1; i++) {
            double esik = (degerler.get(i) + degerler.get(i + 1)) / 2.0;
            esikler.add(esik);
        }
    ...
    private double agirlikliGiniHesapla(List<UserRecord> solGrup, List<UserRecord> sagGrup, int toplamBoyut) {
        double solGini = giniHesapla(solGrup);
        double sagGini = giniHesapla(sagGrup);
        return (solGrup.size() * solGini + sagGrup.size() * sagGini) / toplamBoyut;
    }
    ...
    private double sayisalOzellikDegeriniAl(UserRecord u, int ozellikIndeksi) {
        switch (ozellikIndeksi) {
            ...
            default:
                throw new IllegalArgumentException("Geçersiz sayısal özellik indeksi");
        }
    }
    ...
    private static class Dugum {
        ...
        public Dugum(int ozellikIndeksi, String kategoriDegeri, Dugum solDal, Dugum sagDal) {
            this.yaprakMi = false;
            this.kategorikMi = true;
            this.ozellikIndeksi = ozellikIndeksi;
            this.kategoriDegeri = kategoriDegeri;
            this.solDal = solDal;
            this.sagDal = sagDal;
        }
    }
    ...
    private static class EnIyiBolunme {
        ...
        static EnIyiBolunme sayisalBolunme(int ozellikIndeksi, double esikDegeri, List<UserRecord> solGrup, List<UserRecord> sagGrup) {
            EnIyiBolunme sonuc = new EnIyiBolunme();
            sonuc.ozellikIndeksi = ozellikIndeksi;
            sonuc.esikDegeri = esikDegeri;
            sonuc.kategorikMi = false;
            sonuc.solGrup = solGrup;
            sonuc.sagGrup = sagGrup;
            return sonuc;
        }

        static EnIyiBolunme kategorikBolunme(int ozellikIndeksi, String kategoriDegeri, List<UserRecord> solGrup, List<UserRecord> sagGrup) {
            EnIyiBolunme sonuc = new EnIyiBolunme();
            sonuc.ozellikIndeksi = ozellikIndeksi;
            sonuc.kategorikMi = true;
            sonuc.kategoriDegeri = kategoriDegeri;
            sonuc.solGrup = solGrup;
            sonuc.sagGrup = sagGrup;
            return sonuc;
        }
